...
class Export_Controller extends WP_REST_Controller {
    ...
    public function generate_export() {
        ...
        if ( $zip->open( $zip_path, ZipArchive::CREATE | ZipArchive::OVERWRITE ) !== TRUE ) {
            return new \WP_Error( 'zip_error', 'Could not create zip.', [ 'status' => 500 ] );
        }

        global $wpdb;
        $tables = ['ap_contacts', 'ap_leads', 'ap_invoices', 'ap_gallery_images'];
        ...
        return new \WP_Error( 'export_failed', 'Export failed.', [ 'status' => 500 ] );
    }
}
