...
class Gallery_Controller extends WP_REST_Controller {
    ...
    public function register_routes() {
        register_rest_route( $this->namespace, '/gallery/upload', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ $this, 'handle_upload' ], 'permission_callback' => function() { return current_user_can('manage_options'); }
        ]);
        register_rest_route( $this->namespace, '/gallery/(?P<id>\d+)', [
            'methods' => WP_REST_Server::READABLE, 'callback' => [ $this, 'get_images' ], 'permission_callback' => '__return_true'
        ]);
        register_rest_route( $this->namespace, '/gallery/submit', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ $this, 'submit_selection' ], 'permission_callback' => '__return_true'
        ]);
    }
    public function handle_upload( $request ) {
        ...
        $wpdb->insert("{$wpdb->prefix}ap_gallery_images", [
            'album_id' => $request['album_id'], 'file_name' => $filename, 'file_path' => $target_dir . '/' . $filename,
            'public_url' => $target_url . '/' . $filename, 'serial_number' => '#' . rand(1000,9999), 'proof_id' => '#' . rand(100,999)
        ]);
    ...
    public function submit_selection( $request ) {
        ...
        \AperturePro\Utils\TemplateMailer::send( 'photos_ready', get_option('admin_email'), [] ); // Notify admin
