...
class Questionnaire_Controller extends WP_REST_Controller {
    ...
    public function register_routes() {
        // Admin: Create/Edit Questionnaire Structure
        ...
        // Public: Get Form by ID (to render it)
    ...
    public function create_item( $request ) {
        ...
        $schema = json_encode( $params['questions'] ); 
        
        $wpdb->insert( $wpdb->prefix . 'ap_questionnaires', [
    ...
    public function get_items( $request ) {
        ...
        foreach ( $results as $row ) {
            $row->questions = json_decode( $row->schema_json );
        }
    ...
    public function get_public_item( $request ) {
        ...
        return rest_ensure_response( [
            'id' => $row->id,
            'title' => $row->title,
            'questions' => json_decode( $row->schema_json )
        ] );
    }
    ...
    public function submit_response( $request ) {
        ...
        $wpdb->insert( $wpdb->prefix . 'ap_responses', [
            'questionnaire_id' => intval( $params['form_id'] ),
            'answers_json'     => json_encode( $params['answers'] ),
