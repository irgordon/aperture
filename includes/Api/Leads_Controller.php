...
class Leads_Controller extends WP_REST_Controller {
    ...
    public function register_routes() {
        ...
        register_rest_route( $this->namespace, '/leads/public', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ $this, 'create_public_lead' ], 'permission_callback' => '__return_true'
        ]);
    }
    ...
    public function create_public_lead( $request ) {
        ...
        $wpdb->insert( "{$wpdb->prefix}ap_leads", [
            'contact_id' => $contact_id, 'project_hash' => $hash, 'notes' => sanitize_textarea_field( $request['message'] ), 'source' => 'web'
        ]);
