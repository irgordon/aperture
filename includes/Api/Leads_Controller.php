...
class Leads_Controller extends WP_REST_Controller {
    ...
    public function get_items( $request ) {
        global $wpdb;
        $results = $wpdb->get_results("SELECT l.*, c.first_name, c.last_name FROM {$wpdb->prefix}ap_leads l JOIN {$wpdb->prefix}ap_contacts c ON l.contact_id = c.id ORDER BY l.created_at DESC LIMIT 100");
    ...
    public function create_public_lead( $request ) {
        ...
        return new WP_REST_Response(['message'=>'Success'], 201);
    }
}
