...
class Settings_Controller extends WP_REST_Controller {
    ...
    public function get_settings() {
        ...
        return new WP_REST_Response([
            'branding' => [ 'company_name' => get_option('aperture_company_name'), 'logo_url' => get_option('aperture_logo_url'), 'support_email' => get_option('aperture_support_email'), 'address' => get_option('aperture_company_address'), 'phone' => get_option('aperture_company_phone') ],
            'stripe' => [ 'public_key' => get_option('aperture_stripe_public_key'), 'secret_key' => $secret ? '••••' . substr($secret, -4) : '' ],
            'google' => [ 'client_id' => get_option('ap_google_client_id'), 'client_secret' => get_option('ap_google_client_secret') ? '••••' : '' ],
    ...
    public function test_stripe($request) {
        ...
        try { Stripe::setApiKey($secret); Balance::retrieve(); return new WP_REST_Response(['message'=>'Connected!', 'mode'=>strpos($secret,'sk_test')===0?'TEST':'LIVE']);
        } catch(\Exception $e) { return new \WP_Error('fail', $e->getMessage(), ['status'=>500]); }
    }
    public function get_templates() { global $wpdb; return $wpdb->get_results("SELECT * FROM {$wpdb->prefix}ap_email_templates"); }
    public function update_template($request) { global $wpdb; $p = $request->get_json_params(); $wpdb->update("{$wpdb->prefix}ap_email_templates", ['subject'=>$p['subject'], 'body'=>$p['body']], ['id'=>$p['id']]); return new WP_REST_Response(['success'=>true]); }
